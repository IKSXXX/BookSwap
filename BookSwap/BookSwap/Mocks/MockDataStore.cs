...
public static class MockDataStore
{
    ...
    {
        Books.Clear(); Books.AddRange(await ctx.Books.AsNoTracking()
            .Include(b => b.BookOwners).ThenInclude(bo => bo.User)
            .ToListAsync());

        BookOwners.Clear(); BookOwners.AddRange(await ctx.BookOwners.AsNoTracking()
        ...
        Reviews.Clear(); Reviews.AddRange(await ctx.Reviews.AsNoTracking()
            .Include(r => r.FromUser).Include(r => r.ToUser)
            .ToListAsync());

        Favorites.Clear(); Favorites.AddRange(await ctx.Favorites.AsNoTracking()
        ...
        BooksOfTheDay.Clear(); BooksOfTheDay.AddRange(await ctx.BooksOfTheDay.AsNoTracking()
